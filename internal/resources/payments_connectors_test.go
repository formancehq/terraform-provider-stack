...
func TestPaymentsCreateConfigFromModel(t *testing.T) {
	...
	for _, tc := range []testCase{
		{
			plan: resources.PaymentsConnectorsModel{
				Credentials: types.MapValueMust(types.DynamicType, map[string]attr.Value{
					"apiKey": types.DynamicValue(types.StringValue("my-api-key")),
				}),
				Config: types.MapValueMust(types.DynamicType, map[string]attr.Value{
		...
		t.Run(fmt.Sprintf("%s %s", t.Name(), tc.plan.Config.Elements()["provider"]), func(t *testing.T) {
...
func TestExtractKeys(t *testing.T) {
	...
	for _, tc := range []testCase{
		{
			m: types.MapValueMust(types.DynamicType, map[string]attr.Value{
