...
func TestPaymentsPoolsConnectors(t *testing.T) {
	t.Parallel()
			...
			newTestStepStack(),
			{
				Config: newStack(OrganizationId, RegionName) +
					`
						...
						variable "dummypay_api_key" {
						
						}
						
						resource "formancestack_payments_connectors" "dummypay" {
							credentials = {
						...
						resource "formancestack_payments_pool" "pool1" {
							name = "pool1"
							accounts_ids = [
								"account1",
								"account2",
